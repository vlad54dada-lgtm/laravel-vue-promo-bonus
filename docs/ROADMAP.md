...
**DoD:** тести зелені; другий код у межах 24 год → 409 з часом наступної спроби на формі; через time travel — 200. **Увага для майбутніх демо:** сценарій відео нижче писався до кулдауну — кроки з двома успішними claim підряд тепер вимагають або два акаунти, або зміну `PROMO_COOLDOWN_HOURS=0`. На живому Render-демо виставлено `PROMO_COOLDOWN_HOURS=1` (render.yaml): фіча видима, але спільний акаунт не блокується на добу.
